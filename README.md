...
После изменений в VS Code выполните в терминале проекта:

```powershell
npm run publish
```

Команда сама создаст коммит и отправит файлы в `Ismetkhan1/taxi`. Render автоматически запустит новый деплой. Если интернет временно недоступен, повторите ту же команду позже.
